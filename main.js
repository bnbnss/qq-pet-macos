...
const _debugLog = (() => { try { const fs = require("fs"), os = require("os"); return (msg) => fs.appendFileSync(require("path").join(os.tmpdir(), "pet_crash.log"), new Date().toISOString() + " " + msg + "\n"); } catch(e) { return () => {}; } })();
process.on("uncaughtException", (err) => {
  if (err.code === "EPIPE" || err.message?.includes("EPIPE")) return;
  _debugLog("UNCAUGHT: " + (err.stack || err.message || err));
});
process.on("unhandledRejection", (reason) => {
  _debugLog("REJECT: " + (reason?.stack || reason?.message || reason));
});
